...
sementic_chunks = semantic_chunker.split_text(document)
print(f"Sementic chunks: {len(sementic_chunks)}")
for i, chunk in enumerate(recursive_chunks):
    print(f"\\n--- Chunk {i+1} ({len(chunk)} chars) ---")
    print(chunk[:100] + "..." if len(chunk) > 100 else chunk)
...
sementic_vectorstore = Chroma.from_texts(
    sementic_chunks,
    embeddings,
    collection_name="sementic_chunks"
)
...
for query in test_queries:
    ...
    semantic_result = test_retrieval(query, sementic_vectorstore, 'SEMANTIC')